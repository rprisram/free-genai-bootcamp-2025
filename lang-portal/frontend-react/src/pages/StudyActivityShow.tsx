...
export default function StudyActivityShow() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null)
      try {
        const response = await fetch(`http://localhost:5000/api/study_activities/${id}`)
        if (!response.ok) {
        ...
        const sessionsResponse = await fetch(
          `http://localhost:5000/api/study_activities/${id}/sessions?page=${currentPage}&per_page=${ITEMS_PER_PAGE}`
        )
